...
// ── DATABASE ─────────────────────────────────────────────────────
define('DB_HOST', 'localhost:3307');       // or 'localhost:3307' for XAMPP on custom port
define('DB_USER', 'root');
define('DB_PASS', '');
...
define('SMTP_PASS',      'pfonbtxdcwzyjgww');        // ← Gmail App Password
define('SMTP_FROM_NAME', 'МонголHotels');

// ── QPAY ─────────────────────────────────────────────────────────
define('QPAY_ENV',          'sandbox');
...
class QPay {
    ...
    public static function getToken(): bool {
        ...
        if (!empty($res['access_token'])) { self::$token = $res['access_token']; return true; }
        return false;
    }

    public static function createInvoice(array $booking): array {
        if (!self::getToken()) return ['success' => false];
        $payload = [
        ...
        if (!empty($res['invoice_id'])) {
            return ['success' => true, 'invoice_id' => $res['invoice_id'],
                    'qr_text' => $res['qr_text'] ?? '', 'qr_image' => $res['qr_image'] ?? ''];
        }
